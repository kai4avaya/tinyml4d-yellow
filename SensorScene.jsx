...
const SensorScene = () => {
  ...
  // const [skyOpacity, setSkyOpacity] = useState(0); // Removed state to prevent re-renders loop
  ...
  useEffect(() => {
    ...
    // Function to create matched color and emissive maps for the main board
    ...
    const handleWheel = (e) => {
        ...
        // console.log("Scroll Check:", scrollBottom, docHeight, window.innerHeight);

        // Relaxed check: if we are within the last viewport height of the page
        if (scrollBottom >= docHeight - window.innerHeight + 100) {
           console.log("3D Scroll Active", e.deltaY);
    ...
    const handleTouchMove = (e) => {
         ...
         if (scrollBottom >= docHeight - window.innerHeight + 100) {
            ...
            console.log("3D Touch Active", delta);
    ...
    function animate() {
        ...
        // Debug: Log current scroll to ensure loop is running
        if (Math.random() < 0.01) console.log("Render Active, Scroll:", currentScroll);

        // Update Sky Opacity via Ref (No React Re-render)
        if (skyRef.current) {
            const newOpacity = Math.min(Math.max(currentScroll / 2000, 0), 1);
            skyRef.current.style.opacity = newOpacity;
        }

        // Calculate Rotation
        // Add a base time-based rotation so it's always "alive"
        const time = Date.now() * 0.0005;
        
        // Scroll contribution
        const rotationProgress = currentScroll * 0.002;
        
        // Combine Idle + Scroll rotation
        // Y: Continuous slow spin (idle) + Scroll spin (active) + Wobble (active)
        systemGroup.rotation.y = (time * 0.5) + (rotationProgress) + (Math.sin(rotationProgress) * 0.5); 
        
        // X & Z: Figure-8 wobble primarily driven by scroll, but let's add a tiny idle wobble too
        systemGroup.rotation.x = (Math.sin(time) * 0.05) + (Math.sin(rotationProgress * 0.5) * 0.5);
        systemGroup.rotation.z = (Math.cos(time * 0.7) * 0.05) + (Math.cos(rotationProgress * 0.3) * 0.2);
  ...
  return (
    <div className="relative w-full h-screen cursor-ns-resize overflow-hidden">
        {/* Sky Layer: Fades in to transition to blue */}
        <div 
            ref={skyRef}
            className="absolute inset-0 z-0 pointer-events-none transition-opacity duration-300 ease-out"
            style={{ 
                opacity: 0, // Start transparent
                background: 'linear-gradient(to top, #bae6fd, #3b82f6)' // Sky 200 to Blue 500 (Rising)
            }}
        />
